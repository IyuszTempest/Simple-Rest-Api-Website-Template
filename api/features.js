...
const handleSpotifySearch = async (query) => {
    try {
        // Copy ulang dari dashboard, pastikan TIDAK ADA SPASI di awal/akhir
        const CLIENT_ID = "4bf176753fa74eb46f7c1c6a512dd"; 
        const CLIENT_SECRET = "5ed7ea931e5649f7a73694c20db553e0";

        // Coba pakai URL resmi internasional, jangan pakai link dummy
        ...
        return {
            status: "success",
            author: "IyuszTempest",
            result: searchRes.data.tracks.items.map(t => ({
                name: t.name,
        ...
        throw new Error(`Gagal: ${error.response?.data?.error_description || error.message}`);
    }
};
